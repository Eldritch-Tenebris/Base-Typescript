    ...
    if (guilds?.size) {
        ...
        .then(commands => {
            ...
            messages.push(ck.greenBright(
                `└ ${commands.size} comando${plural(commands.size)} registrado${plural(commands.size)} globalmente!`
            ));
        ...
        for (const guild of guilds.values()) {
            ...
            .then(commands => {
                messages.push(ck.greenBright(
                    `└ ${commands.size} comando${plural(commands.size)} registrado${plural(commands.size)} no servidor ${ck.underline(guild.name)}!`
                ))
                if (baseStorage.config.commands.verbose){
                    messages.push(...verbooseLogs(commands));
                }
    ...
    return commands.map(({ id, name, type: commandType, client, createdAt, guild }) => {
        ...
        return ck.dim.green(
            [
                ...
                ck.gray(">"),
                guild 
                ? `servidor ${u.blue(guild.name)}`
                : `aplicação ${u.blue(client.user.username)}`,
                ck.gray(">"),
                "criado em:",
